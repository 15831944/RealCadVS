...
class EntityHeader
  {protected:
   ...
   public:
	 ...
	 void setRefId(long id)  { refId = id; }
...
protected:
   ...
   public:
     ...
     virtual int move(Database *,Transform &,int erase = 1,int save=1) = 0;
...
private :
   ...
   public :
     ...
     int move(Database *,Transform &,int erase = 1,int save=1);
   ...
   {private:
    ...
    public:
     ...
     void draw(int mode,Transform *t,View3dSurface *, QPainter *) {}
      ...
      Point nearp(Point);
      int stretchent(Database *,Transform &,SortedPointTable *,int erase = 1);
      int move(Database *,Transform &,int,int) {  return 0;  }
...
struct copyAlongDialogData {
	...
	QList<long> copyList;
	int level;
	QMap<int, copyAlongUndoData> undoMap;
	copyAlongDialogData clone(Transform& t, bool pathIsCircle) const;
	QList<long> cloneList(const QList<long>&, Transform& t) const;
	int save(CadFile *);
	int load(CadFile *);
	static Entity* getEntity(int refId);
	void unDo(int undoing,bool isCircle);	
	void move(Transform &t, int erase);
	bool deleteCopyEntities(int id, EntityList& list);
	bool deleteCopyEntity(long id);
	bool insertCopyEntity(long id);
	
	int saveList(CadFile* outfile, const QList<long>& list);
	int loadList(CadFile* infile, QList<long>& list);
};
class LinearEntity;
typedef void(*CopyAlongCallback)(LinearEntity *entity,bool);
...
class Curve : public LinearEntity
  {
    ...
    private:
	 ...
	 int moveImplement(Database *db, Transform &t, int erase, int save);
   public:
...
enum {  UD_INSERT,UD_DELETE,UD_STARTBLOCK,UD_ENDBLOCK,UD_CHANGEHEADER,
        ...
        UD_REPEATCOUNT,UD_BOUNDINGBOX
     };

class GeometryDatabase
{
	friend class Database;	
	friend class copyAlongDialogData;
 private:
...
class LayerNames
  {private:
   ...
   public:
     ...
     void clear(void);
	 bool isEmpty();
	 void copyFrom(const LayerNames& src);
...
{private:
   ...
   CopyAlongCallback CopyAlogEntityCallback;
   ...
   QMap<int, copyAlongDialogData>  copyAlogMap;
   Grid grid;
   Database();
   ~Database();
   void setupunits();
