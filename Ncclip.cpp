...
void undo_command(int *cstate,HighLevelEvent *,void **)
{
	...
	if (cadwindow) {
		...
		//cadwindow->getcurrentwindow()->canShowBoundingBox=false;
	}
#else
  db.undo();
#endif
...
void redo_command(int *cstate,HighLevelEvent *,void **)
{
	...
	if (cadwindow) {
		cadwindow->enddrawingbatch();
		//cadwindow->getcurrentwindow()->canShowBoundingBox = true;
	}
#else
  db.redo();
#endif
...
extern void DeleteCopyEntities(EntityList&  list);
extern bool canDeleteCopyEntities(EntityList&  list);
/**
  ...
  *cstate = ExitState;
  ...
  if (! list.empty())
    {  db.saveundo(UD_STARTBLOCK,NULL);
	   ...
	   if (oldSate != -100) {
		  /* if (!canDeleteCopyEntities(list)) {
			   cadwindow->MessageBox("Original entity can't be deleted!", "Copy Along", MB_ICONINFORMATION);
			   return;
		   }*/
		   DeleteCopyEntities(list);
	   }
	   ...
	   if (db.ShowBoundingBox) {
		   View3dWindow* view3d = cadwindow->getcurrentwindow();
		   view3d->boundingBox.deleteEntity(view3d,list);
	   }
