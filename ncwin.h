...
protected:
     ...
     int israytracing;
 private:
	 int nsegments(double radius, double sweepa, double error);
	 void drawProtractor(BoundingBox&);
	 void drawCircle(const Point& o, double r, double start, double end,bool drawScale=false);
 public:
...
class MainWindow;
class View3dWindow;
struct BoundingBoxDataItem {
	Point minPt;
	Point maxPt;
	Point center;
	double width()  const { return qAbs(maxPt.x - minPt.x); }
	double height() const { return qAbs(maxPt.y - minPt.y); }
};

struct BoundingBoxData {
	BoundingBoxDataItem prevItem;
	BoundingBoxDataItem curntItem;
	QList<long> entityIds;
};

class BoundingBox {
private:	
	enum BoundingBoxDragMode { Undefined = -1, LeftTop, TopEdge, RightTop, RightEdge, RightBottom, BottomEdge, LeftBottom, LeftEdge };
	const int HandleSize = 20;
	bool isProtractor;
	bool ctrlKeyIsPressed;
	BoundingBoxDragMode dragMode;
	
	Point minPt, maxPt;
	Point dragPt1;
	QList<long> entityIds;
	QStack<BoundingBoxData> undoStack;
	QStack<BoundingBoxData> redoStack;
	BoundingBoxDataItem item;
	BoundingBoxDataItem newItem;

private:	
	QRect getLeftTopHandle(View3dSurface*) const;
	QRect getLeftBotHandle(View3dSurface*) const;
	QRect getRightTopHandle(View3dSurface*) const;
	QRect getRightBotHandle(View3dSurface*) const;

	Point leftTopPt(View3dSurface*, bool forCenter=false) const;
	Point leftBotPt(View3dSurface*, bool forCenter=false) const;
	Point rightTopPt(View3dSurface*, bool forCenter=false) const;
	Point rightBotPt(View3dSurface*, bool forCenter=false) const;
	
	Point getOppositePtToSelectPt(View3dSurface *, const Point&) const;
	Point transformToScreen(View3dSurface* s, double x, double y) const;
	
	bool dragLeftTopHandle(View3dSurface*,const Point& p) const;
	bool dragLeftBotHandle(View3dSurface*,const Point& p) const;
	bool dragRightTopHandle(View3dSurface*,const Point& p) const;
	bool dragRightBotHandle(View3dSurface*,const Point& p) const;
	bool ptIsOnEdge(const Point& edgeStart, const Point& edgeEnd, const Point& pt) const;	
	Point getScale(double curWidth, double curHeight, double prevWidth, double prevHeight);
	void getScaleTransfom(int undoing, const BoundingBoxData& data, Transform& t);
	void doScaleTransform(int undoing, const BoundingBoxData& data);
	Point calculateScale(const Point& p);
	Point getCenterPt(View3dWindow* view, const Point& p) const;
	void deleteEntityId(long id);
	EntityList getEntityList(const QList<long>& entityIds);
public:
	BoundingBox();	
	~BoundingBox();
	void undo(int undoing);
	void clear();
	bool isValid();
	bool dragging() const;
	Point centerPt() const;
	Point getMinPt() const { return minPt; }
	Point getMaxPt() const { return maxPt; }
	Point getdragPt1() const {
		return dragPt1;
	}
	EntityList getSelectedList();
	
	Point handleStartPt(View3dWindow*) const;
	Point handleEndPt(View3dWindow*) const;
	Point getRuberBandPt1(View3dWindow* view) const;
	Point getDraggingPt1(View3dWindow*,const Point&) const;
	Point getDraggingPt2(View3dWindow*, const Point&) const;
	bool protractorIsDisplayed() const { return isProtractor; }
	double height() const;
	double width() const;
	void update(const Point& p);
	void updateUndoData();
	void doRotation(View3dWindow* view, const Point& p, double& angle);
	void scaleTransfomList(View3dWindow*,const Point&);
	void getScaleTransfom(View3dWindow* view, const Point& p, Transform& t);
	void getRotateTransfom(View3dWindow* view, const Point& p, Transform& t, double& angle);
	double getRotationHandleLength() const;
	bool rotateHandleIsSelecteded(View3dWindow*,const Point&);
	void onLeftButtonDownEvent(View3dWindow*, const Point& pt, int buttons);
	
	void computeBoundingBox(View3dWindow*, EntityList);
	void deleteEntity(View3dWindow*,EntityList);	
	double getProtractorSize() const;
};
