...
int PointE::move(Database *db,Transform &t,int erase,int save)
{Entity *e;
  if (copye != NULL) return 1;
  for (alist.start() ; (e = alist.next()) != NULL ; )
    if ((e->getstatus() & 1) == 0)
...
void Line::drawGL(int drawmode,Transform *t,View3dSurface *vs)
{
    ...
    */
    ...
    if (gethandles() && !db.ShowBoundingBox )
    {
...
{ 
  ...
  if (getposition() != 0 && db.MovedCallback != 0)
    db.MovedCallback(this);
  if (db.CopyAlogEntityCallback != 0) db.CopyAlogEntityCallback(this,true);
...
int Line::move(Database *db,Transform &t,int erase,int save)
{Entity *e;
  if (copye != NULL) return 1;
  for (alist.start() ; (e = alist.next()) != NULL ; )
    if ((e->getstatus() & 1) == 0)
