...
void transformlist(int project,int ncopies,EntityList list,Transform t,int surface)
{Entity *e;
 EntityList movedlist;
  ...
  if (! project && ncopies == 0)
    {  for (list.start() ; (e = list.next()) != NULL ; )
       ...
       for (list.start() ; (e = list.next()) != NULL ; )
         if (e->getalist().empty()) e->move(&db,t);
       for (list.start() ; (e = list.next()) != NULL ; )
         if (! e->getalist().empty()) e->move(&db,t);
       for (db.geometry.start() ; (e = db.geometry.next()) != NULL ; )
       ...
       case 2 :
         if (event->isa(EntitySelected))
           {  le = (LinearEntity *) ((EntitySelectedEvent *)event)->getentity();
              ...
              xaxis = le->direction(ustart);
              zaxis = xaxis.cross(p1 - le->position(ustart));
              ...
              x1 = xaxis.dot(p1 - le->position(ustart));
              y1 = yaxis.dot(p1 - le->position(ustart));
              z1 = zaxis.dot(p1 - le->position(ustart));

              for (x = spacing ; x <= length + spacing / 1.0E10 ; x += spacing)
                {Transform t1,t2;
                   t1.translate(-p1);

                   le->positionbylength(x,&u);
                   if (ustart != 0.0)
                     u = 1.0 - u;
                   xa = le->direction(u);
                   ya = zaxis.cross(xa);
                   ...
                   if (orientation == 1)
                     {  t2.fromcoordinatesystem(xaxis,yaxis,zaxis);
                        ...
                        t1.apply(t2);
                        t2.translate(le->position(u) + xa * x1 + ya * y1 + zaxis * z1);
                     }
                   else 
                     t2.translate(le->position(u));
