...
{ if (GetLockedLayerTable()->test(layer)) return; 
  ...
  if (db.CopyAlogEntityCallback != 0)
	  db.CopyAlogEntityCallback(this, true);
 ...
 int match1,match2,match3,match4;
  ...
  if ((status & 1) == 1)
    {  if ((status & 16) == 0) draw(DM_ERASE);
       ...
       return moveImplement(db,t,erase,1);
    }
...
//
int Curve::moveImplement(Database *db, Transform &t, int erase, int save) {
	Entity *e;
	int i;
	if (copye != NULL) return 1;
	for (alist.start(); (e = alist.next()) != NULL; )
...
//
int Curve::move(Database *db,Transform &t,int erase,int save)
{
	int result = moveImplement(db, t, erase, save);	
	if (db != 0 && db->CopyAlogEntityCallback != 0 && db->copyAlogMap.contains(this->id)) {
		copyAlongDialogData  dlgData = db->copyAlogMap[this->id];	  
		if (selected) {		
			dlgData.move(t, erase);
			db->CopyAlogEntityCallback(this, true);
		}
	}
	return result;
