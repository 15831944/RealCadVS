...
extern void transformlist(int project,int ncopies,EntityList list,Transform t,int surface,int save=1);
...
int  View3dSurface::nsegments(double radius,double sweepa,double error)
{
	int n;
	double cosa;
...
void  View3dSurface::drawCircle(const Point& o,double r, double start, double end,bool drawScale) {
	double x1 = cos(start);
	double y1 = sin(start);
	
	Point xa = getuaxis() * r;
	Point ya = getvaxis() * r;
	int n = nsegments(r, end-start, (getumax() - getumin()) / getwidth() * db.getdtoler());
	if (drawScale) n = 36;
	double da =(end-start )/ n;
	bool drawCenterCircle = (end - start) < 6.28;
	double sina = sin(da);  
	double cosa = cos(da);
	Point p1 = modeltransform.transform(o + xa * x1 + ya * y1);
	Point pt1 = p1; Point pt2 = p1;
	for (int i = 0; i < n; i++)
	{
		double x2 = x1 * cosa - y1 * sina;
		double y2 = x1 * sina + y1 * cosa;
		Point p2 = modeltransform.transform(o + xa * x2 + ya * y2);
		if(i==n-1) pt2 = p2;		
		if (drawScale) {
			Point dir = (o - p2).normalize();
			Point p3 = p2 + dir * getpixelscale() * 6.0;
			lineGL(p2.x, p2.y, p2.z, p3.x, p3.y, p3.z);
		}
		else
			lineGL(p1.x, p1.y, p1.z, p2.x, p2.y, p2.z);
		p1 = p2;  x1 = x2;  y1 = y2;
	}
	
}
void View3dSurface::drawProtractor( BoundingBox& box) {
	
	double radius =box.getProtractorSize();
	Point origin = box.centerPt();
	glColor3f(0.0f, 0.0f, 1.0f);
	double agl = 6.28 / 36;
	drawCircle(origin, radius, 0, 6.28);
	drawCircle(origin, radius ,0,6.28,true);
	double r = (radius - getpixelscale() * 15.0);
	drawCircle(origin,r, 8*agl, -26*agl);
	double x1 = cos(8 * agl);
	double y1 = sin(8 * agl);
	
	Point p1 = modeltransform.transform(origin + getuaxis() * r * x1 + getvaxis() * r * y1);
	Point p2 = p1;
	
	double r1 = (getuaxis() * r * x1).x;
	p2.x = origin.x - r1;
	Point p3 = origin+ getuaxis()*r1;
	Point p4 = origin- getuaxis()*r1;
	lineGL(p1.x, p1.y, p1.z, p3.x, p3.y, p3.z);	
	lineGL(p2.x, p2.y, p2.z, p4.x, p4.y, p4.z);
	
	drawCircle(origin, r1, 0, -3.14);
	if (box.protractorIsDisplayed()) {
		glEnable(GL_LINE_STIPPLE);
		glLineStipple(1, 0xAAAA); 
		p1 = modeltransform.transform(origin);
		p2 = p1 + getvaxis()*box.getRotationHandleLength();
		lineGL(p1.x, p1.y, p1.z, p2.x, p2.y, p2.z);
...
void View3dSurface::drawBoundingBox(BoundingBox& box) {
	
	Point minP = box.getMinPt();
	Point maxP = box.getMaxPt();
	Point centerP = box.centerPt();
	Point pright = getuaxis();
	Point pup = getvaxis();
	double sc = getpixelscale() * 6.0;

	double boxW = maxP.x - minP.x;
	double boxH = maxP.y - minP.y;
	double z = getdepth();
	Point minPt(minP.x, minP.y, z);
	Point maxPt(maxP.x, maxP.y, z);

	Point leftTop(minP.x, minP.y + boxH,z);
	Point rightBot(maxP.x, maxP.y - boxH, z);
	Point center(minP.x + boxW / 2, minP.y + boxH / 2, 0);
	
	
	glLineWidth(curGLWeight);
	//glLineWidth(1.4);
	//set edge color as blue
	glColor3f(0.0f, 0.0f, 1.0f);//blue
	//draw box side
	movetoGL(minP.x, minP.y,z);
	linetoGL(leftTop.x, leftTop.y,z);
	movetoGL(leftTop.x, leftTop.y, z);
	linetoGL(maxP.x, maxP.y,z);
	movetoGL(maxP.x, maxP.y, z);
	linetoGL(rightBot.x, rightBot.y,z);
	movetoGL(rightBot.x, rightBot.y, z);
	linetoGL(minP.x, minP.y,z);
	 
	//cbox2dGL(center, pright, pup, boxW, boxH);
	
	glColor3f(1.0f, 0.0f, 0.0f);//red

	movetoGL(center.x - sc, center.y,z);
	linetoGL(center.x + sc, center.y,z);
	movetoGL(center.x, center.y - sc,z);
	linetoGL(center.x, center.y + sc,z);

	//draw box vertex with small square;
	cfillbox2dGL(minPt, pright, pup, sc, sc);
	cfillbox2dGL(leftTop, pright, pup, sc, sc);
	cfillbox2dGL(maxPt, pright, pup, sc, sc);
	cfillbox2dGL(rightBot, pright, pup, sc, sc);
	drawProtractor(box);
}
